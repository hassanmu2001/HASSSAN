...
import {
  LayoutDashboard, Users, Package, Shield, LogOut,
  ChevronRight, ChevronLeft, ShieldCheck, Star,
  MessageSquare, FolderOpen, Menu, X, Home
} from "lucide-react";
...
const menuItems = [
  ...
  { title: "الخدمات", url: "/admin/services", icon: Package },
  { title: "الأقسام", url: "/admin/categories", icon: FolderOpen },
  { title: "التقييمات", url: "/admin/reviews", icon: Star },
  { title: "المحادثات", url: "/admin/messages", icon: MessageSquare },
  { title: "التوثيق", url: "/admin/verification", icon: ShieldCheck },
];
...
const AdminLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  if (isMobile) {
    // Bottom 4 quick-access items + more menu
      ...
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <header className="sticky top-0 z-50 bg-purple-deep border-b border-gold/10 flex items-center justify-between px-4 h-14">
          <div className="flex items-center gap-2">
            ...
            <span className="font-amiri text-lg font-bold text-gold">لوحة المشرف</span>
          </div>
  ...
  return (
    <div className="min-h-screen flex w-full bg-background">
      ...
      >
        <div className="h-16 flex items-center justify-center border-b border-gold/10 gap-2">
          <Shield className="w-5 h-5 text-gold shrink-0" />
          {!collapsed && (
            <Link to="/admin" className="font-amiri text-xl font-bold text-gold">
              لوحة المشرف
            </Link>
          )}
        </div>
        ...
        <div className="p-3 border-t border-gold/10 space-y-1">
          <Link
            to="/"
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-primary-foreground/70 hover:bg-gold/10 hover:text-gold w-full transition-colors"
          >
            <LayoutDashboard className="w-5 h-5 shrink-0" />
            {!collapsed && <span>الصفحة الرئيسية</span>}
          </Link>
          <button
            onClick={signOut}
